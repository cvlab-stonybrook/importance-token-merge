...
def matching_by_heat_map(metric: torch.Tensor,
        w: int, h: int, r: int,
    ...
    with torch.no_grad():
        if type(error_mask) == type({}):
            heat_map = error_mask['heat_map']
            if heat_map.shape[0] != (B//2):
                assert heat_map.shape[0] % (B//2) == 0 
                heat_map = heat_map[:B//2]
        else:
            heat_map = error_mask

        B0 = heat_map.shape[0] 
        assert len(heat_map.shape) == 3 # B//2 h w
        if heat_map.shape[1] > h:
            heat_map = F.interpolate(heat_map[:,None,:,:], size=(h, w), mode='area').squeeze(1)
        ...
        if type(error_mask) == type({}):
            heat_idx = error_mask["heat_idx"]
            if heat_idx.shape[0] != (B//2): heat_idx = heat_idx[:B//2]
        else:
            heat_idx = heat_list.argsort(dim=1, descending=True) # B//2, h*w, 1
        ...
        if a.shape[1] <= 9216:
            ...
            node_max, node_idx = chunked_score_max(a, b, chunk_size=a.shape[1]//8)
