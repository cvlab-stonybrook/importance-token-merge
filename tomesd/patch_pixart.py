...
from . import merge_pixart as merge
from .utils import isinstance_str, init_generator


def compute_merge(x: torch.Tensor, tome_info: Dict[str, Any], ratio: float = 0) -> Tuple[Callable, ...]:
    ...
    if downsample <= args["max_downsample"]:
        ...
        original_tome = args["original_tome"]
        if not original_tome: 
            hh, ww = original_h // 2, original_w // 2
            _ijs = torch.stack(torch.meshgrid(torch.arange(hh), torch.arange(ww), indexing='ij'), dim=-1) # h, w, 2
            _ijs = _ijs.reshape(-1, 2).unsqueeze(0) # 1, h*w, 2
            # assert _ijs.shape[1] == x.shape[1]
            ijs = _ijs.to(dtype=x.dtype, device=x.device)
            x = ijs.expand(x.shape[0], -1, -1) # B, h*w,2 

        if args["err_mask"] is not None:
            m, u = merge.matching_by_heat_map_dit(x, w, h, r, generator=args["generator"],
                error_mask=args["err_mask"], pp=1, original_tome=original_tome)
        else:
            m, u = merge.bipartite_soft_matching_random2d_dit(x, w, h, 
                args["sx"], args["sy"], r, no_rand=not use_rand, generator=args["generator"], original_tome=original_tome)
    else:
...
def make_diffusers_tome_block(block_class: Type[torch.nn.Module]) -> Type[torch.nn.Module]:
    ...
    class ToMeBlock(block_class):
        ...
        def forward(
            ...
            # (2) ToMe m_a
            norm_hidden_states = m_a(norm_hidden_states, mode=self._tome_info["args"]["m_mode"])
            ...
            if self.attn2 is not None:
                ...
                # (4) ToMe m_c
                norm_hidden_states = m_c(norm_hidden_states, mode=self._tome_info["args"]["m_mode"])
            ...
            # (6) ToMe m_m
            norm_hidden_states = m_m(norm_hidden_states, mode=self._tome_info["args"]["m_mode"])
...
def apply_patch(
    ...
    for _, module in diffusion_model.named_modules():
        ...
        if isinstance_str(module, "BasicTransformerBlock"):
            block_ii = int(_.split('.')[1])

            if block_ii not in range(7, 21): continue
